...
const UserAvailable = () => {
  const { allUsers, isPending: isGetAllUsersPending } = useGetAllUsers();
  ...
  const availableEmployees = AllEmployees?.map(user => {
    const attendanceRecord = data?.data?.attendance.find(item =>
      item.user._id === user._id
    );
    const isAvailableToday = attendanceRecord?.attendances.some(item =>
